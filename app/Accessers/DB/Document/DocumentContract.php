...
class DocumentContract extends FluentDatabase
{
    ...
    public function getDocumentList(array $mUser, array $condition, array $sort, array $page)
    {
        return $this->builder()
            ...
            ->where('t_document_contract.product_name', 'like', '%'.$condition['product_name'].'%')
            ...
            ->whereRaw('JSON_CONTAINS(t_document_contract.doc_info->"$.title", \'["'.$condition['doc_info']['title'].'"]\')')
            ->whereRaw('JSON_CONTAINS(t_document_contract.doc_info->"$.content", \'["'.$condition['doc_info']['content'].'"]\')')
