...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $e)
    {
        if ($e instanceof ValidationException) {
            ...
            //
            // 【備考】
            // バリデーションエラー：$e->errors()は、上記のようなキーとエラーメッセージの配列形式で格納されている。(*1)
            ...
            //   ]
            //
            // (*1)素の$e->errors()は上記に記載の通りの形式であるが、別途フロント用の加工が必要なため、
            //     ここに来る前段階の各Requestクラスにて加工している。
            //     'document_id' => ['error.message.require']であれば、'error.message.require'の部分を
            //     さらに配列構造にして、可変パラメタの情報を設定している。
            // ----------------------------------------------------------------
